...
from pymilvus import model, Collection
...
# 文件路径
file_path = "D:\\RAG\\rag-project02-medical-nlp-box-master\\rag-project02-medical-nlp-box-master\\backend\\data\\data.csv"
db_path = "D:\\RAG\\rag-project02-medical-nlp-box-master\\rag-project02-medical-nlp-box-master\\backend\\db\\snomed_bge_m3.db"
...
index_params.add_index(
    field_name="vector",  # 指定要为哪个字段创建索引，这里是向量字段
    index_type="IVF_FLAT",  # 使用自动索引类型，Milvus会根据数据特性选择最佳索引
    metric_type="L2",  # 使用余弦相似度作为向量相似度度量方式
    params={"nlist": 1024}  # 索引参数：nlist表示聚类中心的数量，值越大检索精度越高但速度越慢
)
